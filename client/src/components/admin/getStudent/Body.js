...
const Body = () => {
  ...
  const handlePdfDownload = async (pdfBase64, studentName) => {
    const pdfBlob = new Blob([Buffer.from(pdfBase64, 'base64')], { type: 'application/pdf' });
    fileSaver.saveAs(pdfBlob, `${studentName}_details.pdf`);
  };
                  ...
                  students?.length !== 0 && (
                      <div className={classes.adminData}>
                        ...
                        {students?.map((stu, idx) => (
                            <div
                                key={idx}
                                className={`${classes.adminDataBody} grid-cols-11`}
                            >
                              <h1
                              ...
                              <Button
                                  ...
                                  color="primary"
                                  className={`col-span-1 ${classes.adminDataBodyFields}`}
                                  onClick={() => handlePdfDownload(stu.pdf, stu.name)}
                              >
                                Download
                              </Button>
                            </div>
